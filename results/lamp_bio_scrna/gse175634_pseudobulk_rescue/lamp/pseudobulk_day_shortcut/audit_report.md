# LAMP audit report: GSE175634 pseudo-bulk rescue: Pseudo-bulk day shortcut
...
- Output classes: `forbidden_feature_contamination`, `oracle_label_leakage_sentinel`, `protocol_batch_or_donor_shortcut_sentinel`, `visible_state_confounding`

Primary AUC = 0.880; inverted AUC = 0.120. No strong score-direction ambiguity was detected under the default higher-score-means-positive convention.
...
- `structural_endpoint_oracle`: gap `0.1199`, proximity `NA`, Pearson `0.7432`, alert `False`
- `day_protocol`: gap `0.0000`, proximity `NA`, Pearson `1.0000`, alert `False`
- `published_pseudotime`: gap `0.0614`, proximity `NA`, Pearson `0.8772`, alert `False`
- `published_annotation`: gap `0.0673`, proximity `NA`, Pearson `0.6526`, alert `False`
...
- threshold `0.0500`: selected `0.7857`, enrichment `0.0877`
